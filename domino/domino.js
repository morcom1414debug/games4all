import { initializeApp } from "[https://www.gstatic.com/firebasejs/10.8.1/firebase-app.js](https://www.google.com/search?q=https://www.gstatic.com/firebasejs/10.8.1/firebase-app.js)";
import { getDatabase, ref, update, remove, onValue, get } from "[https://www.gstatic.com/firebasejs/10.8.1/firebase-database.js](https://www.google.com/search?q=https://www.gstatic.com/firebasejs/10.8.1/firebase-database.js)";

const firebaseConfig = {
apiKey: "AIzaSyDvcdgsyT5sDdYTYKIqetzNL9Be-MFC0l4",
authDomain: "xo-game-134ec.firebaseapp.com",
databaseURL: "[https://xo-game-134ec-default-rtdb.asia-southeast1.firebasedatabase.app](https://www.google.com/search?q=https://xo-game-134ec-default-rtdb.asia-southeast1.firebasedatabase.app)",
...
list.innerHTML = '';
if (!rooms) {
list.innerHTML = 'ไม่มีห้องที่เปิดอยู่';
return;
}
...
container.className = `domino-tile half-tile ${isSpecial ? 'special-tile ' + spClass : ''}`;
container.setAttribute('aria-hidden', 'true');

const formatHalf = (v) => {
	if(v === 'sleep') return `<div class="domino-half special-text">หลับ<br>💤</div>`;
	if(v === 'draw2') return `<div class="domino-half special-text">2+<br>⚡</div>`;
	if(v === 'draw3') return `<div class="domino-half special-text">3+<br>💥</div>`;
	if(v === 'reverse') return `<div class="domino-half special-text">ย้อนศร<br>🔄</div>`;
	return `<div class="domino-half">${getPipLayoutHTML(v)}</div>`;
};

container.innerHTML = formatHalf(val);
